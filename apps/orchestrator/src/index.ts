...
if (import.meta.main && Bun.argv[2] === "mac-worker") {
  ...
  const driver = new LinuxContainerDriver({ image: Bun.env.MARS_LINUX_CONTAINER_IMAGE!, prefix: "mars-linux-arm64", network: Bun.env.MARS_LINUX_CONTAINER_NETWORK!, limits, jobTimeoutMs: Number(Bun.env.JOB_TIMEOUT_MS ?? 900_000) });
