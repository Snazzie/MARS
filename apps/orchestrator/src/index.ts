...
import { runWindowsWorker, runWindowsWorkerService } from "./windows-agent.ts";
...
if (import.meta.main && Bun.argv[2] === "mac-worker") {
  ...
  if (!baseUrl) throw new Error("WHITESMITH_CONTROL_PLANE_URL is required");
  await (Bun.argv.includes("--service") ? runWindowsWorkerService(baseUrl, limits) : runWindowsWorker(baseUrl, limits));
