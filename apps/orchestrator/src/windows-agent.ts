...
const machineUuid = async () => { const p = Bun.spawn(["powershell.exe", "-NoProfile", "-Command", "(Get-CimInstance Win32_ComputerSystemProduct).UUID"], { stdout: "pipe" }); return (await new Response(p.stdout).text()).trim(); };
const capacity = async (): Promise<WorkerCapacityData> => { const cpu = Number(Bun.env.MAX_VCPU_PER_POD ?? 4), memory = Number(Bun.env.MAX_MEMORY_BYTES_PER_POD ?? 4 * 1024 ** 3), storage = Number(Bun.env.MAX_STORAGE_BYTES_PER_POD ?? 20 * 1024 ** 3); return { actualVcpu: cpu, freeVcpu: cpu, actualMemoryBytes: memory, freeMemoryBytes: memory, actualStorageBytes: storage, freeStorageBytes: storage }; };
