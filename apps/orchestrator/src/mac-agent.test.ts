...
import { availableMacMemoryBytes, buildMacWorkerAuthentication, buildMacWorkerJoinPayload, handleMacWorkerCommand, parseMacWorkerIdentity, runMacLeaseLifecycle, startMacLeaseLifecycle } from "./mac-agent.ts";
...
test("deletes a lease VM and reports reaped when stop says it is already stopped", async () => {
  const calls: string[] = [];
  const driver = {
    async stopLease() { calls.push("stop"); throw new Error("already stopped"); },
  ...
  const result = await handleMacWorkerCommand(command, driver as never);
  expect(calls).toEqual(["stop", "remove"]);
  expect(result).toMatchObject({ type: "lease.reaped", payload: { commandId: command.id, leaseId: command.leaseId, nonce: "n".repeat(32) } });
});
...
test("emits attestation, runner completion, and reap around one lease", async () => {
  ...
  const bootstrap = { jobId: "55555555-5555-4555-8555-555555555555", leaseId: "22222222-2222-4222-8222-222222222222", nonce: "n".repeat(32), imageDigest: "sha256:test", resources: { vcpu: 1, memoryBytes: 2, storageBytes: 3, concurrency: 1 }, encodedJitConfig: "secret", expiresAt: new Date(Date.now() + 60_000).toISOString() };
...
test("streams runner output as unattributed job log events before completion", async () => {
  const sent: Array<{ type: string; payload: Record<string, unknown> }> = [];
  ...
  const driver = {
    async createLease() {
      return { runtimeInstanceId: "vm-1", observed: { vcpu: 1, memoryBytes: 2, storageBytes: 3 }, state: "sandbox_attested" as const, completion: Promise.resolve(0), logs: logs() };
    },
    async stopLease() {},
    async removeLease() {},
  };
  ...
  expect(sent.map(event => event.type)).toEqual(["sandbox_attested", "job.log", "job.log", "runner.finished", "lease.reaped"]);
  expect(sent.filter(event => event.type === "job.log").map(event => event.payload)).toEqual([
    { jobId: bootstrap.jobId, stepId: null, sequence: 0, content: "first output\n", occurredAt: expect.any(String) },
    { jobId: bootstrap.jobId, stepId: null, sequence: 1, content: "second output\n", occurredAt: expect.any(String) },
  ]);
});
