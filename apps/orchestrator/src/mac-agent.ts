...
import { WorkerConfigurePayload, WorkerConfiguration } from "@whitesmith/contracts";
...
export async function runMacWorker(baseUrl: string, limits: MacWorkerLimits): Promise<never> {
  ...
  try {
    const code = codeBytes.toString("utf8").trim();
    const vmUuid = Bun.env.WHITESMITH_VM_UUID ?? hostname();
    const response = await fetch(new URL("/api/workers/join", controlPlane), { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(buildMacWorkerJoinPayload({ code, publicKey: key.publicKey, vmUuid })), signal: AbortSignal.timeout(30_000) });
