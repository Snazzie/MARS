...
describe("HyperVDriver", () => {
  ...
  test("rejects a lease whose image digest is not the sealed template", async () => { const driver = new HyperVDriver(fake([]), "template.vhdx", lease.imageDigest, "mars", limits); await expect(driver.createLease({ ...lease, contractVersion: "0.1.0", imageDigest: "sha256:" + "b".repeat(64) })).rejects.toThrow("does not match"); });
});
