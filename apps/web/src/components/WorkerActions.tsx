...
type Action = "adopt" | "reject" | "drain" | "rotate-key" | "remove";
const copy: Record<Action, { label: string; confirm: string; variant: "primary" | "secondary" | "destructive" }> = {
  adopt: { label: "Adopt", confirm: "Adopt this worker? It will become eligible for configuration once its doctor checks pass.", variant: "primary" },
  reject: { label: "Reject", confirm: "Reject this pending worker? Its enrollment will be revoked and it will not receive work.", variant: "destructive" },
  drain: { label: "Drain", confirm: "Drain this worker? New leases will stop while active work completes.", variant: "secondary" },
  "rotate-key": { label: "Rotate key", confirm: "Rotate this worker's key? The worker must reconnect with the new identity.", variant: "secondary" },
...
export function WorkerActions({ organizationId, workerId, admissionState, draining, onComplete }: { organizationId: string; workerId: string; admissionState: string; draining: boolean; onComplete: () => void }) {
  ...
  return <>
    <div className="worker-actions" aria-label="Worker actions">
      {admissionState === "pending" && <><Button label="Adopt" variant="primary" clickAction={() => open("adopt")} /><Button label="Reject" variant="destructive" clickAction={() => open("reject")} /></>}
      {admissionState === "adopted" && <><Button label={draining ? "Draining" : "Drain"} variant="secondary" isDisabled={draining} clickAction={() => open("drain")} /><Button label="Rotate key" variant="secondary" clickAction={() => open("rotate-key")} /><Button label="Remove" variant="destructive" clickAction={() => open("remove")} /></>}
    </div>
