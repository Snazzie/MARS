...
export function OutcomeBars({ outcomes, label = "Job outcomes" }: { outcomes: readonly JobOutcome[]; label?: string }) {
  ...
  const definition = useMemo(() => defineChart({
    ...
    color: { scale: () => scaleOrdinal<Platform, string>().domain(platformOrder.map(({ key }) => key)).range(["var(--ui-chart-blue)", "var(--ui-chart-orange)", "var(--ui-chart-purple)", "var(--ui-chart-green)"]), legend: colorLegend({ label: "Platform" }) },
    focus: "group-x",
