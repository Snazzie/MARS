...
export function WorkerConfigurationForm({ worker, organizationId, onConfigured, onDiscard }: Props) {
  ...
  return <>
    <form className="worker-configuration-form" onSubmit={submit}>
      <header className="worker-configuration-header"><div><h3>{adopted ? "Configure worker" : "Set worker limits"}</h3><p>{adopted ? "Update the resources this worker may use." : "Confirm the capacity available to jobs, then approve this worker."}</p></div></header>
