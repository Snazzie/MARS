...
export function WorkerConfigurationForm({ worker, organizationId, onConfigured, onDiscard }: Props) {
  ...
  return <>
    ...
    <form className="worker-configuration-form" onSubmit={submit}>
      ...
      <fieldset><legend>Action cache</legend><p className="field-help">Cache entries expire after this many hours. This setting applies to the worker's local cache.</p><label>Cache TTL (hours)<input name="cacheTtlHours" type="number" min="1" step="1" value={cacheTtlHours} onChange={(e) => setCacheTtlHours(e.target.value)} required /></label><label>Cache size (GiB)<input name="runnerCacheMaxGiB" type="number" min="1" step="1" value={runnerCacheMaxGiB} onChange={(e) => setRunnerCacheMaxGiB(e.target.value)} required /></label><label className="checkbox-field"><input name="runnerCacheEnabled" type="checkbox" checked={runnerCacheEnabled} disabled={pending} onChange={(event) => setRunnerCacheEnabled(event.target.checked)} /> Enable worker cache service</label><p className="field-help">When disabled, jobs receive no worker cache or proxy configuration and existing lease credentials are revoked. Stored entries remain available if you re-enable it.</p></fieldset>
