...
export function WorkerConfigurationForm({ worker, organizationId, onConfigured }: Props) {
  ...
  const submit = (event: FormEvent) => {
    ...
    const input: WorkerConfigurationInput = { organizationId, appliance: { vcpu: applianceVcpu, memoryBytes: applianceMemory, storageBytes: applianceStorage }, runtime: { maxVcpuPerPod: podVcpu, maxMemoryBytesPerPod: podMemory, maxStorageBytesPerPod: podStorage, maxConcurrentPods } };
    setError(null);
    setPending(true);
  ...
  return <form className="worker-configuration-form" onSubmit={submit}><p>Configure worker resources and per-job ceilings.</p><p>Configure worker</p>{capacityError && <p role="alert" className="form-error">{capacityError}</p>}{error && <p role="alert" className="form-error">{error}</p>}<label>vCPU<input name="vcpu" type="number" min="1" step="1" value={vcpu} onChange={(e) => setVcpu(e.target.value)} required /></label><label>RAM (GiB)<input name="memoryGiB" type="number" min="1" step="1" value={ram} onChange={(e) => setRam(e.target.value)} required /></label><label>Disk (GiB)<input name="storageGiB" type="number" min="1" step="1" value={disk} onChange={(e) => setDisk(e.target.value)} required /></label><label>Max vCPU per pod<input name="maxVcpuPerPod" type="number" min="1" step="1" value={maxVcpu} onChange={(e) => setMaxVcpu(e.target.value)} required /></label><label>Max RAM per pod (GiB)<input name="maxMemoryGiBPerPod" type="number" min="1" step="1" value={maxRam} onChange={(e) => setMaxRam(e.target.value)} required /></label><label>Max disk per pod (GiB)<input name="maxStorageGiBPerPod" type="number" min="1" step="1" value={maxDisk} onChange={(e) => setMaxDisk(e.target.value)} required /></label><label>Max concurrent pods<input name="maxConcurrentPods" type="number" min="1" step="1" value={concurrency} onChange={(e) => setConcurrency(e.target.value)} required /></label><button type="submit" disabled={pending || Boolean(capacityError)}>{pending ? "Configuring worker…" : "Adopt and configure"}</button></form>;
}
