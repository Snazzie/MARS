...
import type { CostCenterPricePoint, CostCenterPricingProvider } from "@mars/contracts";
...
export function CostCenterPriceChart({ points, provider = "github" }: { points: readonly CostCenterPricePoint[]; provider?: CostCenterPricingProvider }) {
  ...
  return <div className="chart-frame cost-center-price-chart" role="img" aria-label={`Estimated ${providerName} retail cost avoided by completion date. ${summary}`}><Chart definition={definition} height={240} ariaLabel={`Estimated ${providerName} retail cost avoided by completion date`} /></div>;
}
