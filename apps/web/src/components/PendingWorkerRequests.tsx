...
export function PendingWorkerRequests({ organizationId }: Props) {
 const client = useQueryClient(); const query = useQuery({ queryKey: ["pending-workers"], queryFn: getPendingWorkerRequests, enabled: organizationId !== "all", staleTime: 5000 }); const [error,setError]=useState<string|null>(null);
 ...
 if (organizationId === "all") return null;
