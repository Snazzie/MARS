  ...
  return <article className={`worker-card ${worker.admissionState !== "adopted" ? "worker-card-pending" : ""}`} aria-labelledby={`worker-${worker.id}`}>
    <header className="worker-card-header">
      <div className="worker-card-identity">
        ...
        <p className="worker-meta">{worker.platform} · guests: {worker.guestPlatforms.join(", ")} · selected: {worker.selectedDriver ?? "Not selected"} · <span className="worker-fingerprint">key <code tabIndex={0} title={worker.fingerprint}>{worker.fingerprint}</code></span></p>
        {worker.doctor?.capabilities && <p className="worker-meta">Advertised capabilities: {worker.doctor.capabilities.map((capability) => `${capability.driver} (${capability.guestPlatform}${capability.ready ? ", ready" : ", not ready"})`).join("; ") || "None"}</p>}
      </div>
      <div className="worker-card-controls">
        {active && worker.platform === "windows-x64" && worker.runtimeMode !== "vm" && <button type="button" className="control-button" onClick={() => setBuilding(true)} disabled={connectionState !== "online" || worker.doctor?.runtimeBuildState === "building"} title={connectionState === "online" ? "Send the declarative image build to the worker" : "Worker must be online before it can build an image"}>{connectionState !== "online" ? "Worker offline" : worker.doctor?.runtimeBuildState === "building" ? "Building…" : "Build local image"}</button>}
        <WorkerActions organizationId={organizationId} workerId={worker.id} admissionState={worker.admissionState} draining={worker.draining} activeSandboxes={worker.activeSandboxes} platform={worker.platform} currentReleaseVersion={worker.releaseVersion} currentContractVersion={worker.contractVersion} onComplete={onChange} />
