...
export function WorkerCard({ worker, organizationId, onChange }: { worker: WorkerDetail; organizationId: string; onChange: () => void }) {
  ...
  return <article className={`worker-card ${worker.admissionState !== "adopted" ? "worker-card-pending" : ""}`} aria-labelledby={`worker-${worker.id}`}>
    <header className="worker-card-header"><div><div className="worker-name-row"><span className={`status-dot status-${worker.connectionState}`} aria-label={worker.connectionState} /><h2 id={`worker-${worker.id}`}>{worker.name}</h2></div><p className="worker-meta">{worker.platform} · {worker.driver} · <span className={`status-pill status-${worker.admissionState}`}>{worker.admissionState}</span></p><div className="worker-statuses" aria-label="Worker status"><span className={`status-pill status-${worker.connectionState}`} aria-label={`Operational status: ${workerOperationalLabel(worker)}`}>{workerOperationalLabel(worker)}</span><span className={`status-pill status-${worker.configurationState}`} aria-label={`Readiness status: ${workerReadinessLabel(worker.configurationState)}`}>{workerReadinessLabel(worker.configurationState)}</span></div></div><div className="worker-actions">{active && worker.configurationState !== "ready" && <Button label="Configure" variant="primary" clickAction={openConfiguration} />}<WorkerActions organizationId={organizationId} workerId={worker.id} admissionState={worker.admissionState} draining={worker.draining} onComplete={onChange} /></div></header>
    ...
    <dialog ref={dialog} className="worker-config-dialog" onCancel={closeConfiguration} aria-label="Configure worker">{configuring && <WorkerConfigurationForm worker={{ id: worker.id, capacity: capacityData, limits: worker.limits }} organizationId={organizationId} onConfigured={() => { closeConfiguration(); onChange(); }} />}</dialog>
  </article>;
}
