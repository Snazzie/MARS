...
export function WorkerCard({ worker, organizationId, onChange }: { worker: WorkerDetail; organizationId: string; onChange: () => void }) {
  ...
  return <article className={`worker-card ${worker.admissionState !== "adopted" ? "worker-card-pending" : ""}`} aria-labelledby={`worker-${worker.id}`}>
    ...
    {active && worker.platform === "windows-x64" && !runtimeReady && <button type="button" className="control-button" onClick={() => setBuilding(true)} disabled={worker.connectionState !== "online"} title={worker.connectionState === "online" ? "Send the declarative image build to the worker" : "Worker must be online before it can build an image"}>{worker.connectionState === "online" ? "Build local image" : "Worker offline"}</button>}
    ...
    {active && effectiveConfigurationState === "ready" && !runtimeReady && <p className="pending-note" role="status">Runtime image is not ready. Scheduling remains paused until the worker reports a verified local runtime.</p>}
