...
import type { RunnerWorkflowPreview } from "@whitesmith/contracts";
...
export function RunnerWorkflowPrModal({ organizationId, repositoryId, repositoryName, open, onClose, onCreated }: Props) {
  ...
  const jobs = useMemo(() => preview.data?.jobs ?? [], [preview.data]);
  if (!open) return null;
  const invalid = !preview.data || preview.data.noOp || preview.data.replacementCount === 0 || preview.isFetching || files.isLoading || create.isPending;
  return <div className="modal-backdrop" role="presentation"><section className="runner-workflow-modal" role="dialog" aria-modal="true" aria-labelledby="runner-workflow-title">
    ...
    {error && <div role="alert" className="form-error">{error instanceof Error ? error.message : "Could not load workflow preview."}<button type="button" className="control-button" onClick={() => void preview.refetch()}>Refresh preview</button></div>}
    ...
    {files.data && <><fieldset className="workflow-file-selection"><legend>Workflow files</legend>{files.data.length === 0 && <p>No eligible workflow files found.</p>}{files.data.map((file) => <label key={file.path}><input type="checkbox" checked={selected.includes(file.path)} onChange={(e) => setSelected((current) => e.target.checked ? [...current, file.path] : current.filter((path) => path !== file.path))} /> <code>{file.path}</code></label>)}</fieldset>
      {preview.isFetching && <p role="status" aria-live="polite">Refreshing preview…</p>}
      {preview.data && <div className="workflow-job-list" aria-live="polite"><h3>Runner changes</h3>{jobs.length === 0 ? <p>No jobs with runs-on values are selected.</p> : jobs.map((job) => <article key={`${job.path}:${job.id}`}><strong>{job.id}</strong><small>{job.path}</small><p><code>{Array.isArray(job.currentRunsOn) ? job.currentRunsOn.join(", ") : job.currentRunsOn}</code> → <code>{job.proposedRunsOn.join(", ")}</code></p></article>)}</div>}
      <label>PR title<input value={title} maxLength={200} onChange={(e) => setTitle(e.target.value)} /></label><label>PR description<textarea value={body} maxLength={10000} onChange={(e) => setBody(e.target.value)} /></label>
