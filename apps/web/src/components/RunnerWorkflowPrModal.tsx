...
export function RunnerWorkflowPrModal({
  ...
  return <div className="modal-backdrop" role="presentation"><section ref={dialogRef} tabIndex={-1} className="runner-workflow-modal" role="dialog" aria-modal="true" aria-labelledby="runner-workflow-title">
    ...
    {error && <div role="alert" className="form-error">{error instanceof Error ? error.message : "Could not load workflow preview."}<button type="button" className="control-button control-button-secondary" onClick={() => { create.reset(); void (files.error ? files.refetch() : preview.refetch()); }}>Refresh preview</button></div>}
