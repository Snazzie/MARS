...
export const isRunnerWorkflowPrDisabled = (input: { result: string | null; hasPreview: boolean; noOp?: boolean; replacementCount?: number; previewLoading?: boolean; filesLoading?: boolean; submitting?: boolean; confirmed?: boolean }) => Boolean(input.result) || !input.hasPreview || Boolean(input.noOp) || input.replacementCount === 0 || !input.confirmed || Boolean(input.previewLoading || input.filesLoading || input.submitting);
...
export function RunnerWorkflowPrModal({ organizationId, repositoryId, repositoryName, open, onClose, onCreated }: Props) {
  ...
  const [selected, setSelected] = useState<string[]>([]); const [title, setTitle] = useState(""); const [body, setBody] = useState(""); const [result, setResult] = useState<string | null>(null); const [confirmed, setConfirmed] = useState(false);
  useEffect(() => {
    if (!open) return;
  ...
  useEffect(() => { if (files.data) { setSelected(files.data.map((f) => f.path)); setConfirmed(false); } }, [files.data]);
  ...
  if (!open) return null;
  const invalid = isRunnerWorkflowPrDisabled({ result, hasPreview: Boolean(preview.data), noOp: preview.data?.noOp, replacementCount: preview.data?.replacementCount, previewLoading: preview.isFetching, filesLoading: files.isLoading, submitting: create.isPending, confirmed });
  return <div className="modal-backdrop" role="presentation"><section ref={dialogRef} tabIndex={-1} className="runner-workflow-modal" role="dialog" aria-modal="true" aria-labelledby="runner-workflow-title">
    ...
    {files.data && <><fieldset className="workflow-file-selection"><legend>Workflow files</legend>{files.data.length === 0 && <p>No eligible workflow files found.</p>}{files.data.map((file) => <label key={file.path}><input type="checkbox" checked={selected.includes(file.path)} onChange={(e) => setSelected((current) => e.target.checked ? [...current, file.path] : current.filter((path) => path !== file.path))} /> <code>{file.path}</code></label>)}</fieldset>
      ...
      <label>PR title<input value={title} maxLength={200} onChange={(e) => setTitle(e.target.value)} /></label><label>PR description<textarea value={body} maxLength={10000} onChange={(e) => setBody(e.target.value)} /></label><label><input type="checkbox" checked={confirmed} onChange={(e) => setConfirmed(e.target.checked)} /> I confirm the selected workflow replacements.</label>{result && <p role="status" aria-live="polite">Pull request created. <a href={result} target="_blank" rel="noreferrer">Open pull request</a></p>}
