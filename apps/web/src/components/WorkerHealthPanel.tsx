...
function CacheSection({ health, idPrefix, cacheMetrics }: { health: WorkerHealth; idPrefix: string; cacheMetrics?: ReactNode }) {
  ...
  return <section className="worker-health-section" aria-labelledby={`${idPrefix}-cache-heading`}>
    ...
    <dl className="worker-health-details">
      ...
      <div><dt>Cache service enabled</dt><dd>{cache.runnerCacheObservedAt == null ? "Not reported" : cache.effectiveRunnerCacheEnabled == null ? "Not reported" : cache.effectiveRunnerCacheEnabled ? "Enabled" : "Disabled"}</dd></div>
      <div><dt>Cache capacity</dt><dd>{cache.runnerCacheObservedAt == null ? "Not reported" : cache.effectiveRunnerCacheMaxGiB == null ? "Not reported" : `${cache.effectiveRunnerCacheMaxGiB} GiB`}</dd></div>
