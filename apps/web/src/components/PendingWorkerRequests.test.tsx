...
test("keeps identity and capacity controls inside each review, with one rejection action", () => { const html = markup([request, { ...request, id: "55555555-5555-4555-8555-555555555555", vmUuid: "66666666-6666-4666-8666-666666666666" }]); expect(html.match(/class="pending-worker-review"/g)?.length).toBe(2); expect(html.match(/<summary>Review and approve/g)?.length).toBe(2); expect(html.match(/Reject worker<\/button>/g)?.length).toBe(2); expect(html).toContain(request.publicKey); expect(html).toContain(request.fingerprint); expect(html.match(/name="vcpu"/g)?.length).toBe(2); expect(html.match(/name="maxConcurrentPods"/g)?.length).toBe(2); });
