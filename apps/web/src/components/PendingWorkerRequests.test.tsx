...
test("renders independent per-job limits and separate worker scheduling", () => { const html = markup(); expect(html).toContain(request.publicKey); expect(html).toContain("Approve and configure worker"); expect(html).toContain("Worker capacity"); expect(html).toContain("Per-job limits"); expect(html).toContain("Worker scheduling"); expect(html).toContain("Jobs are admitted dynamically"); expect(html.match(/name=\"vcpu\"/g)?.length).toBe(1); expect(html.match(/name=\"maxConcurrentPods\"/g)?.length).toBe(1); });
