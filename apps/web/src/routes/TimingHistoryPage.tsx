...
export function TimingHistoryPage() {
  ...
  return <>
    ...
    {items.length > 0 && <section aria-label="Completed timing measurements" className="panel"><h2>Completed measurements</h2><table><thead><tr><th>Job</th><th>Completed</th><th>Execution</th><th>Total</th><th>Resources</th></tr></thead><tbody>{items.map(item => <tr key={item.jobId}><td>{item.repositoryName} / {item.jobName}</td><td>{new Date(item.completedAt).toLocaleString()}</td><td>{item.executionDurationMs} ms</td><td>{item.totalDurationMs} ms</td><td>{item.requestedVcpu} vCPU / {item.effectiveConcurrency} parallel</td></tr>)}</tbody></table></section>}
