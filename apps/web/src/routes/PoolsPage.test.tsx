...
test("counts compatible shared-pool workers by operational and readiness status", () => {
  const coverage = poolWorkerCoverage({ platform: "macos-arm64", driver: "tart-vm", workerId: null, imageDigest: "macos" }, [
    { id: "w1", name: "worker-1", platform: "macos-arm64", driver: "tart-vm", artifactDigest: "macos", artifactDigests: { "macos-arm64": "macos" }, connectionState: "online", configurationState: "ready", configurationRevision: "a", appliedConfigurationRevision: "a", draining: false, doctor: { runtimeReady: true, probe: true, imageSignatures: true } },
...
test("does not mark a configured worker ready when its runtime doctor fails", () => {
  const coverage = poolWorkerCoverage({ platform: "macos-arm64", driver: "tart-vm", workerId: null, imageDigest: "macos" }, [
    { id: "w1", name: "worker-1", platform: "macos-arm64", driver: "tart-vm", artifactDigest: "macos", artifactDigests: { "macos-arm64": "macos" }, connectionState: "online", configurationState: "ready", configurationRevision: "a", appliedConfigurationRevision: "a", draining: false, doctor: { runtimeReady: false } },
  ]);
