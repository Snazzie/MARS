...
function ReviewSummary({ detail, through, onClose }: { detail: OnboardingDetail; through: number; onClose?: () => void }) {
  ...
  return <aside className="onboarding-review">
    ...
    {through >= 1 && detail.worker && <p>Worker enrollment<br />Worker: {detail.worker.name ?? detail.worker.vmUuid}</p>}
    {through >= 2 && detail.github.organizationId && <p>GitHub account: {org?.name ?? detail.github.organizationId}<br />Repositories: {detail.github.repositories.filter((r) => r.approved).map((r) => r.name).join(", ") || "Pending approval"}</p>}
  </aside>;
}
...
function GithubStep({ detail, onApprove }: { detail: OnboardingDetail; onApprove: (ids: string[]) => void }) {
  const [selected, setSelected] = useState<string[]>([]);
  const [organizationId, setOrganizationId] = useState(detail.github.organizationId ?? "");
  const [connectError, setConnectError] = useState<string | null>(null);
  const repos = detail.github.repositories.filter((repository) => repository.available && repository.visibility !== "public");
  const allRepositories = detail.github.installation?.repositorySelection === "all";
  const hasUsableInstallation = Boolean(detail.github.installation && repos.length > 0);
  ...
  return <div>
    ...
    {selectionRemediation && <p role="alert">The GitHub App is installed with all repositories. Reconnect and choose Only select repositories, then approve at least one available private or internal repository.</p>}
    {allRepositories && <p className="warning-note">This installation grants all repositories. You can still approve specific available repositories below.</p>}
    {hasUsableInstallation && <form onSubmit={(event) => { event.preventDefault(); if (selected.length) onApprove(selected); }}>
      <p>Select the available private or internal repositories Whitesmith may schedule.</p>
      {repos.map((repository) => <label key={repository.id}><input type="checkbox" name={repository.id} checked={selected.includes(repository.id)} onChange={(event) => setSelected(event.target.checked ? [...selected, repository.id] : selected.filter((id) => id !== repository.id))} />{repository.name} ({repository.visibility})</label>)}
      <button type="submit" disabled={!selected.length}>Approve repositories</button>
    </form>}
  </div>;
}
