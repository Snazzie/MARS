...
export function OnboardingPage() {
  ...
  return <main className="onboarding"><header><p className="eyebrow">FIRST-RUN SETUP</p><h1>Get Whitesmith ready</h1><p>Complete each verified step. Progress is saved on the control plane.</p></header>{error && <p role="alert" className="form-error">{error} <button onClick={() => setError(null)}>Dismiss</button></p>}<div className="onboarding-layout"><nav aria-label="Onboarding steps"><ol className="onboarding-steps">{steps.map(([id, label], index) => <li key={id} className={index === currentIndex ? "is-current" : index < currentIndex ? "is-complete" : "is-locked"}><span>{index + 1}</span>{index < currentIndex ? <button type="button" aria-label={`Edit ${label} step`} onClick={() => setEditingStep(index)}>{label}</button> : <strong>{label}</strong>}</li>)}</ol></nav><section className="onboarding-task" aria-live="polite"><h2>{editingStep !== null ? `Edit ${steps[editingStep]?.[1]} step` : steps[currentIndex]?.[1]}</h2>{editingStep !== null ? <><button type="button" onClick={() => setEditingStep(null)}>Back to current step</button><EditableStep detail={d} index={editingStep} onClose={() => setEditingStep(null)} onDone={() => { setEditingStep(null); refresh(); }} onSelect={(id) => select.mutate({ workerId: id })} onCreate={(input) => pool.mutate({ ...input, organizationId: d.github.organizationId ?? "" })} /></> : <>{currentIndex > 0 && <ReviewSummary detail={d} through={currentIndex} />}{d.step === "worker" && <WorkerSetupStep detail={d} onSelect={(id) => select.mutate({ workerId: id })} onDone={refresh} />}{d.step === "github" && <GithubStep detail={d} />}{d.step === "labels" && <LabelsStep detail={d} onCreate={(input) => pool.mutate({ ...input, organizationId: d.github.organizationId ?? "" })} />}</>}</section></div></main>;
}
function EditableStep({ detail, index, onClose, onDone, onSelect, onCreate }: { detail: OnboardingDetail; index: number; onClose: () => void; onDone: () => void; onSelect: (id: string) => void; onCreate: (input: CreatePoolRequest) => void }) {
  if (index === 1) return <WorkerSetupStep detail={detail} edit onSelect={onSelect} onDone={onDone} />;
  if (index === 2) return <GithubStep detail={detail} edit />;
  if (index === 3) return <LabelsStep detail={detail} edit onCreate={onCreate} />;
  return <ReviewSummary detail={detail} through={1} onClose={onClose} />;
}
