...
export async function runGuestService(platform: "windows-x64" | "linux-x64", bootstrapPath: string, runnerRoot: string): Promise<void> {
  ...
  if (bootstrap.version !== 1 || !bootstrap.leaseId || !bootstrap.nonce || !bootstrap.encodedJitConfig || !bootstrap.callbackUrl || !bootstrap.callbackToken) throw new Error("guest bootstrap invalid");
