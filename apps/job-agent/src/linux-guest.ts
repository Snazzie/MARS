...
export async function runLinuxVirtioGuestStream(channel: Duplex, runnerRoot: string, now = () => Date.now(), workerCache?: WorkerCacheProxy): Promise<number> {
  ...
  for await (const chunk of channel.readable) {
    for (const message of parser.push(chunk)) {
      ...
      const exitCode = await runRunnerWithWorkerCache(envelope.encodedJitConfig, runnerRoot, "linux-x64", workerCache, (stream, content) => {
        channel.write(encodeLinuxGuestMessage({ type: "job.log", leaseId, stream, content: content.slice(0, 256 * 1024), sequence: sequence++ }));
      });
