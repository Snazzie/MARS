...
test("seals and opens a lease JIT bootstrap without plaintext payload", () => {
  ...
  const envelope = { leaseId: "11111111-1111-4111-8111-111111111111", jobId: "33333333-3333-4333-8333-333333333333", nonce: "n".repeat(32), encodedJitConfig: "secret", expiresAt: new Date(Date.now() + 60_000).toISOString(), imageDigest: "sha256:test", resources: { vcpu: 1, memoryBytes: 1024, storageBytes: 1024, concurrency: 1 } };
...
test("dispatches ciphertext under a non-secret command field", async () => {
  ...
  const keys = generateKeyPairSync("x25519");
  const publicKey = keys.publicKey.export({ format: "pem", type: "spki" }).toString();
  await dispatchLeaseBootstrap({ dispatch: async (input: unknown) => { calls.push(input); } }, { leaseId: "11111111-1111-4111-8111-111111111111", jobId: "33333333-3333-4333-8333-333333333333", workerId: "22222222-2222-4222-8222-222222222222", workerEncryptionPublicKey: publicKey, imageDigest: "sha256:test", resources: { vcpu: 1, memoryBytes: 1024, storageBytes: 1024, concurrency: 1 }, nonce: "n".repeat(32), encodedJitConfig: "secret", expiresAt: new Date(Date.now() + 60_000).toISOString() });
