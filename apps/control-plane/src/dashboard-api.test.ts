...
function appFor(user = member, db = fakeDb()) { return createControlPlaneApp({ db, setup: { publicOrigin: () => "https://x", configure: async (_code, origin) => origin, authorize: async () => true, claimAdmin: async () => "admin" }, browserOrigin: () => "https://x", githubApp: { getOAuthCredentials: async () => ({ clientId: "id", clientSecret: "secret" }), getWebhookSecret: async () => "webhook" } as never, secretBox: new SecretBox(Buffer.alloc(32, 7).toString("base64")), defaultJobImages: {}, requestId: () => "req", requestSource: () => "test", webRoot: new URL("file:///tmp/"), workerInstallerRoot: new URL("file:///tmp/"), workerOrchestratorExecutable: new URL("file:///tmp/whitesmith-orchestrator"), onWorkerAdopted: () => {}, health: () => ({ buildId: "test-build", startedAt: "2026-08-13T00:00:00.000Z", discovery: { lastAttemptAt: null, lastSuccessAt: null, stale: false, staleAfterMs: 60_000 } }), currentUser: async () => user }); }
