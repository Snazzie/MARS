...
test("reserves a runner slot before requesting JIT config", async () => {
  ...
  expect(routingKey).toBe("acme/project:4:arm64,macos,self-hosted,whitesmith-default");
});
...
test("uses every available pool slot for one installation", async () => {
  ...
  const result = await reconcileQueuedJobs({
    ...
    candidates: [{ requestedLabels: [], worker: { id: "worker", admissionState: "adopted", connectionState: "online", configurationState: "ready", configurationRevision: "current", appliedConfigurationRevision: "current", limits: { maxVcpuPerPod: 4, maxMemoryBytesPerPod: 100, maxStorageBytesPerPod: 100, maxConcurrentPods: 3 } }, pool: { id: "pool", enabled: true, resources: { vcpu: 1, memoryBytes: 1, storageBytes: 1, concurrency: 3 }, concurrency: 3, active: 0, labels: ["whitesmith-windows-x64"], triggerLabel: "whitesmith-windows-x64" } }],
