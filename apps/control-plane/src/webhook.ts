...
export async function acceptDelivery(sql: Sql<{}>, deliveryId:string, installationId:number, payload:unknown, eventName = "unknown"): Promise<boolean> {
  return sql.begin(async tx => {
    const inserted = await tx`insert into webhook_deliveries (delivery_id,installation_id,payload,event_name,state) values (${deliveryId},${installationId},${JSON.stringify(payload)},${eventName},'received') on conflict (delivery_id) do nothing returning delivery_id`;
    if (!inserted.length) {
