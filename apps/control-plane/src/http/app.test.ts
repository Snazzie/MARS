...
const app = createControlPlaneApp(fakeHttpDeps());
  ...
  test.each([
    "https://github.com/Snazzie/Mars/releases/latest/download/windows-worker-checkpoint.zip",
    "https://github.com/Snazzie/Mars/releases/download/v1/windows-worker-checkpoint.zip",
...
test("maps a stale GitHub installation token to not found", async () => {
  ...
  const response = await createControlPlaneApp(fakeHttpDeps({
    db,
    currentUser: async () => ({ id: "admin", githubUserId: 1, login: "admin", isGlobalAdmin: true }),
    githubApp: { getInstallationRateLimit: async () => { throw new Error("github_404"); } } as never,
...
test("maps invalid GitHub rate-limit headers to an upstream error", async () => {
  ...
  const response = await createControlPlaneApp(fakeHttpDeps({
    db,
    currentUser: async () => ({ id: "admin", githubUserId: 1, login: "admin", isGlobalAdmin: true }),
    githubApp: { getInstallationRateLimit: async () => { throw new Error("github_rate_limit_invalid"); } } as never,
  })).request("/api/organizations/org-1/github/rate-limit");
