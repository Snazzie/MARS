...
export function registerWorkerRoutes(app: Hono<ControlPlaneEnv>, deps: ControlPlaneHttpDeps) {
  ...
  app.get("/api/workers/installer", async (c) => {
    ...
    const browserOrigin = deps.browserOrigin() ?? origin;
    return new Response(injectInstallerOrigin(await installer.text(), browserOrigin, extra, audience === "windows-x64"), { headers: noStore() });
  });
