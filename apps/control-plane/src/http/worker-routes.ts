...
export function registerWorkerRoutes(app: Hono<ControlPlaneEnv>, deps: ControlPlaneHttpDeps) {
  ...
  app.post("/api/workers/enroll", async (c) => { const user = await auth(c); if (!user) return c.json({ error: "unauthorized" }, 401); if (!user.isGlobalAdmin) return c.json({ error: "forbidden" }, 403); if (!idempotency(c)) return c.json({ error: "Idempotency-Key required" }, 400); const result = await initializeWorkerBootstrap(deps.db, user.id); const audience = c.req.query("audience") ?? (await c.req.json<{ audience?: string }>()).audience; return c.json({ ...result, expiresAt: new Date(Date.parse(result.createdAt) + 15 * 60_000).toISOString(), installer: `${deps.baseUrl}/api/workers/installer${audience ? `?audience=${encodeURIComponent(audience)}` : ""}` }, { status: 201, headers: noStore() }); });
