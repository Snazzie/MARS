...
export function registerDashboardRoutes(app: Hono<ControlPlaneEnv>, deps: ControlPlaneHttpDeps) {
  ...
  app.get("/api/organizations/:organizationId/cost-center", safe(async (c) => { const org = c.req.param("organizationId"); const period = periodSchema.safeParse(c.req.query("period") || "24h"); if (!period.success) return error(c, 400, "invalid_period", "Invalid period", { issues: period.error.issues }); if (org !== "all") { const denied = await guard(c, deps, org); if (denied) return denied; } const data = await getGithubRunnerCostCenter(deps.db, org, period.data, c.get("user").id); return c.json(CostCenterDto.parse({ organizationId: org, period: period.data, ...data })); }));
