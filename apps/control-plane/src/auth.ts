...
export async function getSession(sql: Sql<{}>, token: string | undefined): Promise<SessionUser | null> { if (!token) return null; const [row] = await sql<SessionUser[]>`select u.id,u.github_user_id as "githubUserId",u.login,u.is_global_admin as "isGlobalAdmin" from sessions s join users u on u.id=s.user_id where s.token_hash=${sha256(Buffer.from(token,"base64url"))} and s.expires_at>now()`; return row ?? null; }
