...
test.skipIf(!databaseUrl)("queued webhook becomes a real lease and encrypted worker dispatch", async () => {
  ...
  await db`insert into workers (id, name, platform, admission_state, connection_state, configuration_state, encryption_public_key, limits, vm_uuid, machine_uuid) values (${workerId}, 'e2e-worker', 'linux-x64', 'adopted', 'online', 'ready', ${workerPublicKey}, ${JSON.stringify({ maxVcpuPerPod: 2, maxMemoryBytesPerPod: 4 * 1024 ** 3, maxStorageBytesPerPod: 8192, maxConcurrentPods: 1 })}, ${crypto.randomUUID()}, ${crypto.randomUUID()})`;
  ...
  const result = await runQueuedJobReconciliation({
    ...
    githubFetchForInstallation: () => async (_input, init) => {
      if (init?.method === "POST") {
        ...
        return Response.json({ encoded_jit_config: "encoded-jit-config" });
      }
  ...
  const [lease] = await db`select state, github_job_id, expires_at, requested from runner_leases where github_job_id=987654321`;
