...
test.skipIf(!databaseUrl)("queued webhook becomes a real lease and encrypted worker dispatch", async () => {
  ...
  await db`insert into dashboard_repositories (id, organization_id, installation_id, github_repository_id, name, full_name, visibility, available, approved) values (${repositoryId}, ${org.id}, ${installationId}, ${Math.floor(Math.random() * 1_000_000_000)}, 'repo', 'e2e-org/repo', 'private', true, true)`;
