...
const event = (commandId: string) => ({ version: 1, id: "33333333-3333-4333-8333-333333333333", workerId, type: "command.accepted", occurredAt: new Date().toISOString(), payload: { commandId, leaseId } });
describe("worker command dispatch", () => {
  test("rejects unauthenticated worker and acknowledges after durable send", async () => { const dispatcher = new WorkerCommandDispatcher(); await expect(dispatcher.dispatch({ workerId, type: "tart.create_lease", leaseId, payload: {} })).rejects.toBeInstanceOf(WorkerDispatchError); let sent = ""; dispatcher.register(workerId, { send: value => { sent = value; } }); const accepted = await dispatcher.dispatch({ workerId, type: "tart.create_lease", leaseId, payload: {} }); const command = JSON.parse(sent); expect(command).toMatchObject({ version: 1, workerId, leaseId, type: "tart.create_lease" }); expect(command.id).toMatch(/[0-9a-f-]{36}/); expect(accepted).toMatchObject({ type: "command.accepted", payload: { commandId: command.id, leaseId } }); });
  test("rejects unauthenticated worker and reports socket send failures", async () => { const dispatcher = new WorkerCommandDispatcher(5); dispatcher.register(workerId, { send: () => { throw new Error("closed"); } }); await expect(dispatcher.dispatch({ workerId, type: "tart.stop_lease", leaseId, payload: {} })).rejects.toThrow("send failed"); });
