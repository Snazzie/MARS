...
async function getOverviewRunningContainers(db: DashboardDb, organizationId: string, userId?: string): Promise<NonNullable<OverviewDto["runningContainers"]>> {
  ...
  const rows = userId
    ? await db<Record<string, unknown>[]>`SELECT l.id,j.id AS "jobId",j.name AS "jobName",r.full_name AS "repositoryName",dr.workflow_name AS "workflowName",w.name AS "workerName",p.driver AS runtime,COALESCE(l.updated_at,j.started_at,l.created_at) AS "startedAt",s.cpu_usage_percent AS "cpuUsagePercent",s.memory_working_set_bytes AS "memoryWorkingSetBytes",COALESCE(NULLIF(s.memory_limit_bytes,0),NULLIF((l.requested->>'memoryBytes')::bigint,0),NULLIF((j.requested->>'memoryBytes')::bigint,0),NULLIF((p.resources->>'memoryBytes')::bigint,0)) AS "memoryLimitBytes",NULL::bigint AS "diskUsageBytes",COALESCE(NULLIF((l.requested->>'storageBytes')::bigint,0),NULLIF((j.requested->>'storageBytes')::bigint,0),NULLIF((p.resources->>'storageBytes')::bigint,0),0) AS "allocatedStorageBytes",s.occurred_at AS "sampledAt" FROM runner_leases l JOIN memberships m ON m.organization_id=l.organization_id AND m.user_id=${userId} JOIN dashboard_jobs j ON j.organization_id=l.organization_id AND j.github_job_id=l.github_job_id JOIN dashboard_runs dr ON dr.organization_id=j.organization_id AND dr.id=j.run_id JOIN dashboard_repositories r ON r.organization_id=dr.organization_id AND r.id=dr.repository_id JOIN workers w ON w.id=l.worker_id JOIN runner_pools p ON p.id=l.pool_id LEFT JOIN LATERAL (SELECT cpu_usage_percent,memory_working_set_bytes,memory_limit_bytes,occurred_at FROM dashboard_job_resource_samples WHERE organization_id=j.organization_id AND job_id=j.id AND lease_id=l.id ORDER BY occurred_at DESC LIMIT 1) s ON true WHERE l.state=ANY(${activeStates}) ORDER BY COALESCE(l.updated_at,j.started_at,l.created_at) DESC,l.id DESC`
    : await db<Record<string, unknown>[]>`SELECT l.id,j.id AS "jobId",j.name AS "jobName",r.full_name AS "repositoryName",dr.workflow_name AS "workflowName",w.name AS "workerName",p.driver AS runtime,COALESCE(l.updated_at,j.started_at,l.created_at) AS "startedAt",s.cpu_usage_percent AS "cpuUsagePercent",s.memory_working_set_bytes AS "memoryWorkingSetBytes",COALESCE(NULLIF(s.memory_limit_bytes,0),NULLIF((l.requested->>'memoryBytes')::bigint,0),NULLIF((j.requested->>'memoryBytes')::bigint,0),NULLIF((p.resources->>'memoryBytes')::bigint,0)) AS "memoryLimitBytes",NULL::bigint AS "diskUsageBytes",COALESCE(NULLIF((l.requested->>'storageBytes')::bigint,0),NULLIF((j.requested->>'storageBytes')::bigint,0),NULLIF((p.resources->>'storageBytes')::bigint,0),0) AS "allocatedStorageBytes",s.occurred_at AS "sampledAt" FROM runner_leases l JOIN dashboard_jobs j ON j.organization_id=l.organization_id AND j.github_job_id=l.github_job_id JOIN dashboard_runs dr ON dr.organization_id=j.organization_id AND dr.id=j.run_id JOIN dashboard_repositories r ON r.organization_id=dr.organization_id AND r.id=dr.repository_id JOIN workers w ON w.id=l.worker_id JOIN runner_pools p ON p.id=l.pool_id LEFT JOIN LATERAL (SELECT cpu_usage_percent,memory_working_set_bytes,memory_limit_bytes,occurred_at FROM dashboard_job_resource_samples WHERE organization_id=j.organization_id AND job_id=j.id AND lease_id=l.id ORDER BY occurred_at DESC LIMIT 1) s ON true WHERE l.organization_id=${organizationId} AND l.state=ANY(${activeStates}) ORDER BY COALESCE(l.updated_at,j.started_at,l.created_at) DESC,l.id DESC`;
  return rows.map((row) => ({
    id: String(row.id),
