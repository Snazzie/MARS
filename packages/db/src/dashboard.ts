...
import type { ActionGraph, CursorPage, LogChunk, OrganizationSummary, OverviewDto, OverviewTimeseriesPoint, RepositorySummary, RunDetail, RunJob, RunStep, RunStage, RunStageRecord, RunSummary, WorkerDetail, OrganizationSettings } from "@whitesmith/contracts";
...
export async function getRunDetail(db: DashboardDb, organizationId: string, runId: string): Promise<RunDetail | null> { const runPage = await listRuns(db, organizationId, 1000); const [run] = runPage.items.filter(x => x.id === runId); if (!run) return null; const jobRows = await db<Record<string, unknown>[]>`SELECT id,name,status,conclusion,stage,runner_name AS "runnerName",logs_state AS "logsState",requested,requested_labels AS "requestedLabels",observed FROM dashboard_jobs WHERE organization_id=${organizationId} AND run_id=${runId} ORDER BY id`; const jobs = await Promise.all(jobRows.map(async (job): Promise<RunJob> => { const requestedLabels = jsonValue(job.requestedLabels); const stepRows = await db<Record<string, unknown>[]>`SELECT id,name,number,status,conclusion,queued_at AS "queuedAt",started_at AS "startedAt",completed_at AS "completedAt",duration_ms AS "durationMs" FROM dashboard_job_steps WHERE organization_id=${organizationId} AND run_id=${runId} AND job_id=${String(job.id)} ORDER BY number,id`; const steps: RunStep[] = stepRows.map(step => normalizeStep(step)); return { ...job, id: String(job.id), name: String(job.name), status: job.status as RunJob["status"], conclusion: job.conclusion == null ? null : String(job.conclusion), stage: job.stage as RunJob["stage"], runnerName: job.runnerName == null ? null : String(job.runnerName), logsState: job.logsState as RunJob["logsState"], requested: jsonValue(job.requested) as RunJob["requested"], requestedLabels: Array.isArray(requestedLabels) ? requestedLabels.map(String) : [], observed: job.observed === null ? null : jsonValue(job.observed) as RunJob["observed"], steps }; })); const stages = await db<RunStageRecord[]>`SELECT stage,started_at AS "startedAt",completed_at AS "completedAt",0::bigint AS "durationMs" FROM dashboard_run_stages WHERE organization_id=${organizationId} AND run_id=${runId} ORDER BY started_at`; return { ...run, jobs, stages, actionGraph: { nodes: stages.map(s => ({ id: s.stage, name: s.stage, status: s.stage })), edges: [] } } as RunDetail; }
function normalizeLogChunk(row: LogChunk): LogChunk {
  return { ...row, sequence: Number(row.sequence), occurredAt: normalizeTimestamp(row.occurredAt)! };
}
export async function listStepLogChunks(db: DashboardDb, organizationId: string, runId: string, jobId: string, stepId: string, after = -1, limit = 100): Promise<CursorPage<LogChunk>> { const safeLimit = Math.max(0, Math.min(1000, Math.floor(limit))); if (safeLimit === 0) return { items: [], nextCursor: null }; const rows = await db<LogChunk[]>`SELECT organization_id AS "organizationId", run_id AS "runId", job_id AS "jobId", sequence, content, false AS "hasMore", occurred_at AS "occurredAt" FROM dashboard_step_log_chunks WHERE organization_id=${organizationId} AND run_id=${runId} AND job_id=${jobId} AND step_id=${stepId} AND sequence>${after} ORDER BY sequence LIMIT ${safeLimit + 1}`; return { items: rows.slice(0, safeLimit).map(x => ({ ...normalizeLogChunk(x), hasMore: rows.length > safeLimit, content: x.content.slice(0, 256 * 1024) })), nextCursor: rows.length > safeLimit ? String(rows[safeLimit - 1].sequence) : null }; }
export async function listLogChunks(db: DashboardDb, organizationId: string, runId: string, jobId: string, after = -1, limit = 100): Promise<CursorPage<LogChunk>> { const rows = await db<LogChunk[]>`SELECT organization_id AS "organizationId", run_id AS "runId", job_id AS "jobId", sequence, content, false AS "hasMore", occurred_at AS "occurredAt" FROM dashboard_log_chunks WHERE organization_id=${organizationId} AND run_id=${runId} AND job_id=${jobId} AND sequence>${after} ORDER BY sequence LIMIT ${limit + 1}`; return { items: rows.slice(0, limit).map(x => ({ ...normalizeLogChunk(x), hasMore: rows.length > limit })), nextCursor: rows.length > limit ? String(rows[limit - 1].sequence) : null }; }
function normalizeStep(row: Record<string, unknown>): RunStep { const queuedAt = normalizeTimestamp(row.queuedAt)!; const startedAt = normalizeTimestamp(row.startedAt); const completedAt = normalizeTimestamp(row.completedAt); const stored = Number(row.durationMs); return { ...row, id: String(row.id), number: Number(row.number), queuedAt, startedAt, completedAt, durationMs: stored > 0 ? stored : derivedDurationMs(startedAt, completedAt, row.status) } as RunStep; }
