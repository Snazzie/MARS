...
function workerCache(row: Record<string, unknown>): WorkerCacheSummary {
  ...
  return WorkerCacheSummary.parse({
    ...
    runnerCacheSizeBytes: row.runnerCacheObservedAt == null ? null : String(row.runnerCacheSizeBytes ?? "0"),
    runnerCacheEntryCount: row.runnerCacheObservedAt == null ? null : Number(row.runnerCacheEntryCount ?? 0),
