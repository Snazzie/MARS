...
export async function listAllRepositories(db: DashboardDb, userId: string, limit = 50): Promise<CursorPage<RepositorySummary>> { const rows = await db<Record<string, unknown>[]>`SELECT r.id, r.organization_id AS "organizationId", r.name, r.full_name AS "fullName", r.visibility, r.available, r.installation_id AS "installationId", CASE WHEN r.discovery_error='github_403' AND r.discovery_retry_at>now() THEN 'paused' WHEN r.discovery_error='github_403' AND r.discovery_retry_at<=now() THEN 'queued' ELSE 'active' END AS "discoveryState", r.discovery_retry_at AS "discoveryRetryAt" FROM dashboard_repositories r JOIN memberships m ON m.organization_id=r.organization_id AND m.user_id=${userId} JOIN dashboard_installations i ON i.organization_id=r.organization_id AND i.id=r.installation_id ORDER BY r.full_name LIMIT ${limit + 1}`; const items = rows.slice(0, limit).map(normalizeRepository); return { items, nextCursor: rows.length > limit ? String(items.at(-1)?.id) : null }; }
