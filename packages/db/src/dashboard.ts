...
async function getOverviewRunningContainers(db: DashboardDb, organizationId: string, userId?: string): Promise<NonNullable<OverviewDto["runningContainers"]>> {
  ...
  const rows = userId
    ? await db<Record<string, unknown>[]>`SELECT l.id,j.id AS "jobId",j.name AS "jobName",r.full_name AS "repositoryName",dr.workflow_name AS "workflowName",w.name AS "workerName",p.driver AS runtime,COALESCE(l.updated_at,j.started_at,l.created_at) AS "startedAt",s.cpu_usage_percent AS "cpuUsagePercent",s.memory_working_set_bytes AS "memoryWorkingSetBytes",COALESCE(NULLIF(s.memory_limit_bytes,0),NULLIF((((CASE WHEN jsonb_typeof(l.requested)='string' THEN (l.requested#>>'{}')::jsonb ELSE l.requested END)->>'memoryBytes')::bigint),0),NULLIF((((CASE WHEN jsonb_typeof(j.requested)='string' THEN (j.requested#>>'{}')::jsonb ELSE j.requested END)->>'memoryBytes')::bigint),0),NULLIF((((CASE WHEN jsonb_typeof(p.resources)='string' THEN (p.resources#>>'{}')::jsonb ELSE p.resources END)->>'memoryBytes')::bigint),0)) AS "memoryLimitBytes",NULL::bigint AS "diskUsageBytes",COALESCE(NULLIF((((CASE WHEN jsonb_typeof(l.requested)='string' THEN (l.requested#>>'{}')::jsonb ELSE l.requested END)->>'storageBytes')::bigint),0),NULLIF((((CASE WHEN jsonb_typeof(j.requested)='string' THEN (j.requested#>>'{}')::jsonb ELSE j.requested END)->>'storageBytes')::bigint),0),NULLIF((((CASE WHEN jsonb_typeof(p.resources)='string' THEN (p.resources#>>'{}')::jsonb ELSE p.resources END)->>'storageBytes')::bigint),0),0) AS "allocatedStorageBytes",s.occurred_at AS "sampledAt" FROM runner_leases l JOIN memberships m ON m.organization_id=l.organization_id AND m.user_id=${userId} JOIN dashboard_jobs j ON j.organization_id=l.organization_id AND j.github_job_id=l.github_job_id JOIN dashboard_runs dr ON dr.organization_id=j.organization_id AND dr.id=j.run_id JOIN dashboard_repositories r ON r.organization_id=dr.organization_id AND r.id=dr.repository_id JOIN workers w ON w.id=l.worker_id JOIN runner_pools p ON p.id=l.pool_id LEFT JOIN LATERAL (SELECT cpu_usage_percent,memory_working_set_bytes,memory_limit_bytes,occurred_at FROM dashboard_job_resource_samples WHERE organization_id=j.organization_id AND job_id=j.id AND lease_id=l.id ORDER BY occurred_at DESC LIMIT 1) s ON true WHERE l.state=ANY(${activeStates}) AND j.status NOT IN ('completed') AND j.completed_at IS NULL ORDER BY COALESCE(l.updated_at,j.started_at,l.created_at) DESC,l.id DESC`
    : await db<Record<string, unknown>[]>`SELECT l.id,j.id AS "jobId",j.name AS "jobName",r.full_name AS "repositoryName",dr.workflow_name AS "workflowName",w.name AS "workerName",p.driver AS runtime,COALESCE(l.updated_at,j.started_at,l.created_at) AS "startedAt",s.cpu_usage_percent AS "cpuUsagePercent",s.memory_working_set_bytes AS "memoryWorkingSetBytes",COALESCE(NULLIF(s.memory_limit_bytes,0),NULLIF((((CASE WHEN jsonb_typeof(l.requested)='string' THEN (l.requested#>>'{}')::jsonb ELSE l.requested END)->>'memoryBytes')::bigint),0),NULLIF((((CASE WHEN jsonb_typeof(j.requested)='string' THEN (j.requested#>>'{}')::jsonb ELSE j.requested END)->>'memoryBytes')::bigint),0),NULLIF((((CASE WHEN jsonb_typeof(p.resources)='string' THEN (p.resources#>>'{}')::jsonb ELSE p.resources END)->>'memoryBytes')::bigint),0)) AS "memoryLimitBytes",NULL::bigint AS "diskUsageBytes",COALESCE(NULLIF((((CASE WHEN jsonb_typeof(l.requested)='string' THEN (l.requested#>>'{}')::jsonb ELSE l.requested END)->>'storageBytes')::bigint),0),NULLIF((((CASE WHEN jsonb_typeof(j.requested)='string' THEN (j.requested#>>'{}')::jsonb ELSE j.requested END)->>'storageBytes')::bigint),0),NULLIF((((CASE WHEN jsonb_typeof(p.resources)='string' THEN (p.resources#>>'{}')::jsonb ELSE p.resources END)->>'storageBytes')::bigint),0),0) AS "allocatedStorageBytes",s.occurred_at AS "sampledAt" FROM runner_leases l JOIN dashboard_jobs j ON j.organization_id=l.organization_id AND j.github_job_id=l.github_job_id JOIN dashboard_runs dr ON dr.organization_id=j.organization_id AND dr.id=j.run_id JOIN dashboard_repositories r ON r.organization_id=dr.organization_id AND r.id=dr.repository_id JOIN workers w ON w.id=l.worker_id JOIN runner_pools p ON p.id=l.pool_id LEFT JOIN LATERAL (SELECT cpu_usage_percent,memory_working_set_bytes,memory_limit_bytes,occurred_at FROM dashboard_job_resource_samples WHERE organization_id=j.organization_id AND job_id=j.id AND lease_id=l.id ORDER BY occurred_at DESC LIMIT 1) s ON true WHERE l.organization_id=${organizationId} AND l.state=ANY(${activeStates}) AND j.status NOT IN ('completed') AND j.completed_at IS NULL ORDER BY COALESCE(l.updated_at,j.started_at,l.created_at) DESC,l.id DESC`;
  return rows.map((row) => ({
    id: String(row.id),
    ...
    sampledAt: row.sampledAt instanceof Date ? row.sampledAt.toISOString() : row.sampledAt == null ? null : String(row.sampledAt),
    cpuUsagePercent: row.cpuUsagePercent == null ? null : Number(row.cpuUsagePercent),
    memoryWorkingSetBytes: row.memoryWorkingSetBytes == null ? null : Number(row.memoryWorkingSetBytes),
    memoryLimitBytes: row.memoryLimitBytes == null ? null : Number(row.memoryLimitBytes),
    diskUsageBytes: row.diskUsageBytes == null ? null : Number(row.diskUsageBytes),
    allocatedStorageBytes: Number(row.allocatedStorageBytes ?? 0),
  }));
}
