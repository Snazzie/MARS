...
function normalizeOverviewTimeseries(rows: Array<Record<string, unknown>>): OverviewTimeseriesPoint[] {
  return rows.map((row) => ({ bucket: row.bucket instanceof Date ? row.bucket.toISOString() : String(row.bucket), pending: Number(row.pending ?? 0), running: Number(row.running ?? 0) }));
}
...
async function getOverviewTimeseries(db: DashboardDb, period: OverviewDto["period"], organizationId: string, userId?: string): Promise<OverviewTimeseriesPoint[]> {
  const rows = userId
    ? await db<Record<string, unknown>[]>`WITH buckets AS (SELECT generate_series(date_trunc(CASE ${period} WHEN '24h' THEN 'hour' ELSE 'day' END, now() - CASE ${period} WHEN '24h' THEN interval '24 hours' WHEN '7d' THEN interval '7 days' ELSE interval '30 days' END), date_trunc(CASE ${period} WHEN '24h' THEN 'hour' ELSE 'day' END, now()), CASE ${period} WHEN '24h' THEN interval '1 hour' ELSE interval '1 day' END) AS bucket) SELECT b.bucket, count(j.id) FILTER (WHERE r.queued_at <= b.bucket AND (r.started_at IS NULL OR r.started_at > b.bucket) AND (r.completed_at IS NULL OR r.completed_at > b.bucket))::int AS pending, count(j.id) FILTER (WHERE r.started_at IS NOT NULL AND r.started_at <= b.bucket AND (r.completed_at IS NULL OR r.completed_at > b.bucket))::int AS running FROM buckets b LEFT JOIN dashboard_runs r ON r.organization_id IN (SELECT organization_id FROM memberships WHERE user_id=${userId}) AND r.queued_at >= now() - CASE ${period} WHEN '24h' THEN interval '24 hours' WHEN '7d' THEN interval '7 days' ELSE interval '30 days' END LEFT JOIN dashboard_jobs j ON j.organization_id=r.organization_id AND j.run_id=r.id GROUP BY b.bucket ORDER BY b.bucket`
    : await db<Record<string, unknown>[]>`WITH buckets AS (SELECT generate_series(date_trunc(CASE ${period} WHEN '24h' THEN 'hour' ELSE 'day' END, now() - CASE ${period} WHEN '24h' THEN interval '24 hours' WHEN '7d' THEN interval '7 days' ELSE interval '30 days' END), date_trunc(CASE ${period} WHEN '24h' THEN 'hour' ELSE 'day' END, now()), CASE ${period} WHEN '24h' THEN interval '1 hour' ELSE interval '1 day' END) AS bucket) SELECT b.bucket, count(j.id) FILTER (WHERE r.queued_at <= b.bucket AND (r.started_at IS NULL OR r.started_at > b.bucket) AND (r.completed_at IS NULL OR r.completed_at > b.bucket))::int AS pending, count(j.id) FILTER (WHERE r.started_at IS NOT NULL AND r.started_at <= b.bucket AND (r.completed_at IS NULL OR r.completed_at > b.bucket))::int AS running FROM buckets b LEFT JOIN dashboard_runs r ON r.organization_id=${organizationId} AND r.queued_at >= now() - CASE ${period} WHEN '24h' THEN interval '24 hours' WHEN '7d' THEN interval '7 days' ELSE interval '30 days' END LEFT JOIN dashboard_jobs j ON j.organization_id=r.organization_id AND j.run_id=r.id GROUP BY b.bucket ORDER BY b.bucket`;
  return normalizeOverviewTimeseries(rows);
}
