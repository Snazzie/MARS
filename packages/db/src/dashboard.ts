...
export async function getAllOverview(db: DashboardDb, userId: string, period: OverviewDto["period"]): Promise<OverviewDto> {
  const [row] = await db<OverviewDto[]>`SELECT 'all'::text AS "organizationId", ${period}::text AS period, count(*) FILTER (WHERE r.status='queued')::int AS queued, count(*) FILTER (WHERE r.status='in_progress')::int AS running, count(*) FILTER (WHERE r.status='completed' AND r.conclusion='success')::int AS completed, count(*) FILTER (WHERE r.status='completed' AND r.conclusion <> 'success')::int AS failed, 0::int AS "queueP50Ms", 0::int AS "queueP95Ms", 0::int AS "durationP50Ms", 0::int AS "durationP95Ms", 0::int AS concurrency, 0::float AS utilization FROM dashboard_runs r JOIN memberships m ON m.organization_id=r.organization_id AND m.user_id=${userId} WHERE r.queued_at >= now() - CASE ${period} WHEN '24h' THEN interval '24 hours' WHEN '7d' THEN interval '7 days' ELSE interval '30 days' END`;
  return { ...row, utilization: { vcpu: 0, memory: 0, storage: 0, pods: 0 } };
}
...
export async function listPools(db: DashboardDb, organizationId: string, limit = 50): Promise<CursorPage<PoolSummary>> {
  const rows = await db<PoolSummary[]>`SELECT p.id,p.organization_id AS "organizationId",p.worker_id AS "workerId",w.name AS "workerName",p.name,p.platform,p.driver,p.image_digest AS "imageDigest",p.resources,p.labels,p.trigger_label AS "triggerLabel",p.enabled,0::int AS active FROM runner_pools p JOIN workers w ON w.organization_id=p.organization_id AND w.id=p.worker_id WHERE p.organization_id=${organizationId} ORDER BY p.name LIMIT ${limit + 1}`;
