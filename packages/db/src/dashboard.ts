...
export async function getOverview(db: DashboardDb, organizationId: string, period: OverviewDto["period"]): Promise<OverviewDto> {
  const [row] = await db<OverviewDto[]>`SELECT ${organizationId}::text AS "organizationId", ${period}::text AS period, count(*) FILTER (WHERE j.status='queued')::int AS queued, (SELECT count(*)::int FROM runner_leases l JOIN dashboard_jobs active_j ON active_j.github_job_id=l.github_job_id WHERE active_j.organization_id=${organizationId} AND l.state IN ('sandbox_ready','online','busy')) AS running, count(*) FILTER (WHERE j.status='completed' AND j.conclusion='success')::int AS completed, count(*) FILTER (WHERE j.status='completed' AND j.conclusion <> 'success')::int AS failed, 0::int AS "queueP50Ms", 0::int AS "queueP95Ms", 0::int AS "durationP50Ms", 0::int AS "durationP95Ms", COALESCE((SELECT sum((p.resources->>'concurrency')::int)::int FROM runner_pools p WHERE p.enabled=true AND (p.organization_id IS NULL OR p.organization_id=${organizationId})),0)::int AS concurrency FROM dashboard_jobs j WHERE j.organization_id=${organizationId}`;
...
export async function getAllOverview(db: DashboardDb, userId: string, period: OverviewDto["period"]): Promise<OverviewDto> {
  const [row] = await db<OverviewDto[]>`SELECT 'all' AS "organizationId", ${period}::text AS period, count(*) FILTER (WHERE j.status='queued')::int AS queued, (SELECT count(*)::int FROM runner_leases l JOIN dashboard_jobs active_j ON active_j.github_job_id=l.github_job_id JOIN memberships am ON am.organization_id=active_j.organization_id AND am.user_id=${userId} WHERE l.state IN ('sandbox_ready','online','busy')) AS running, count(*) FILTER (WHERE j.status='completed' AND j.conclusion='success')::int AS completed, count(*) FILTER (WHERE j.status='completed' AND j.conclusion <> 'success')::int AS failed, 0::int AS "queueP50Ms", 0::int AS "queueP95Ms", 0::int AS "durationP50Ms", 0::int AS "durationP95Ms", COALESCE((SELECT sum((p.resources->>'concurrency')::int)::int FROM runner_pools p WHERE p.enabled=true AND (p.organization_id IS NULL OR EXISTS (SELECT 1 FROM memberships pm WHERE pm.organization_id=p.organization_id AND pm.user_id=${userId}))),0)::int AS concurrency FROM dashboard_jobs j JOIN memberships m ON m.organization_id=j.organization_id AND m.user_id=${userId}`;
...
function workerCache(row: Record<string, unknown>): WorkerCacheSummary {
  ...
  return WorkerCacheSummary.parse({
    ...
    runnerCacheSizeBytes: String(row.runnerCacheSizeBytes ?? "0"),
    runnerCacheEntryCount: Number(row.runnerCacheEntryCount ?? 0),
    observedAt: row.cacheObservedAt == null ? null : normalizeTimestamp(row.cacheObservedAt),
    runnerCacheObservedAt: row.runnerCacheObservedAt == null ? null : normalizeTimestamp(row.runnerCacheObservedAt),
...
export async function getWorkerHealth(db: DashboardDb, workerId: string, workerConnected: (workerId: string) => boolean): Promise<WorkerHealth | null> {
  ...
  return WorkerHealth.parse({
    ...
    cache: {
      ...
      runnerCacheSizeBytes: healthDecimal(worker.runnerCacheSizeBytes),
      runnerCacheEntryCount: healthNumber(worker.runnerCacheEntryCount),
