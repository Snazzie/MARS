...
describe("onboarding state derivation", () => {
  ...
  test("completes only after a successful smoke run is reaped", async () => {
    ...
    const db = (async (strings: TemplateStringsArray) => {
      ...
      if (query.includes("FROM workers w JOIN system_onboarding")) return [{ id: workerId, name: "windows-worker", platform: "windows-x64", guestPlatforms: ["windows-x64"], releaseVersion: "1.0.0", contractVersion: "0.1.0", admissionState: "adopted", connectionState: "online", configurationState: "ready", desiredConfiguration: { selectedDriver: "windows-hyperv-container" }, publicKey: "public", fingerprint: "fingerprint", vmUuid: workerId, machineUuid: workerId, doctorObservedAt: new Date(), doctor: { doctor: { capabilities: [{ driver: "windows-hyperv-container", guestPlatform: "windows-x64", ready: true, imageDigest: `sha256:${"a".repeat(64)}`, remediation: null }] }, capacity: { actualVcpu: 4, actualMemoryBytes: 8_589_934_592, actualStorageBytes: 42_949_672_960, freeVcpu: 4, freeMemoryBytes: 8_589_934_592, freeStorageBytes: 42_949_672_960 } }, limits: { maxVcpuPerPod: 2, maxMemoryBytesPerPod: 4_294_967_296, maxStorageBytesPerPod: 21_474_836_480, maxConcurrentPods: 1 }, configurationRevision: "a".repeat(64), appliedConfigurationRevision: "a".repeat(64) }];
      if (query.includes("SELECT organization_id AS")) return [{ organizationId }];
      if (query.includes("FROM runner_pools") && normalized.includes("p.organization_id is null")) return [{ id: poolId, organizationId: null, workerId: null, workerName: "Shared fleet", name: "default", platform: "windows-x64", driver: "windows-hyperv-container", imageDigest: `sha256:${"a".repeat(64)}`, resources: { vcpu: 2, memoryBytes: 2_147_483_648, storageBytes: 10_737_418_240, concurrency: 1 }, cpuMode: "shared", labels: ["self-hosted", "windows", "x64", "mars-default"], triggerLabel: "mars-default", enabled: true, active: 0 }];
