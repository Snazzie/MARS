...
export const OnboardingDetail = OnboardingStatus.extend({ worker:OnboardingWorker.nullable(), organizations:z.array(OrganizationSummary), github:z.object({ appConfigured:z.boolean(), organizationId:z.string().uuid().nullable(), installation:OnboardingInstallation.nullable(), repositories:z.array(RepositorySummary) }).strict(), pool:PoolSummary.nullable(), verification:OnboardingVerification.default({ state:"not_started", repositoryId:null, poolId:null, workflowPath:null, githubRunId:null, runId:null, error:null }), defaultImageDigests:z.record(GuestPlatform,z.string().nullable()).default({}), defaultImageDigest:z.string().nullable().optional(), ubuntuVersion:z.enum(["22", "24", "26"]).default("24") }).strict();
