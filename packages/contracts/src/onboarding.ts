import { z } from "zod";
import { RuntimePlatform, WorkerState, ConnectionState, ConfigurationState, WorkerDoctorData, WorkerCapacityData, WorkerLimits } from "./orchestration.ts";
...
export const OnboardingWorker = z.object({ id:z.string().uuid(), name:z.string().min(1), platform:RuntimePlatform, admissionState:WorkerState, connectionState:ConnectionState, configurationState:ConfigurationState, publicKey:z.string(), fingerprint:z.string(), vmUuid:z.string(), machineUuid:z.string(), doctor:WorkerDoctorData, capacity:WorkerCapacityData, limits:WorkerLimits.nullable(), configurationRevision:z.string().nullable() }).strict();
