...
export const RunJob = dto(strict({ id, name: z.string().min(1), status: z.enum(["queued", "in_progress", "completed"]), conclusion: z.string().nullable(), stage: RunStage, runnerName: z.string().nullable(), requested: resources, requestedLabels: z.array(z.string().min(1)), observed: resources.nullable(), steps: z.array(RunStep) }));
