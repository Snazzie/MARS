...
export const RepositorySummary = dto(strict({ id, organizationId, name: z.string().min(1), fullName: z.string().min(1), visibility: z.enum(["private", "internal", "public"]), available: z.boolean(), approved: z.boolean(), installationId: id }));
...
export const RunJob = dto(strict({ id, name: z.string().min(1), status: z.enum(["queued", "in_progress", "completed"]), conclusion: z.string().nullable(), stage: RunStage, runnerName: z.string().nullable(), requested: resources, requestedLabels: z.array(z.string().min(1)), observed: resources.nullable() }));
