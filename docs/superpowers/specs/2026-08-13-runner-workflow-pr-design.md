# Whitesmith Runner Workflow Pull Requests

## Goal

Let global administrators create a GitHub pull request that switches selected repository workflow jobs to the configured Whitesmith runner labels. The action is available from onboarding and the dashboard repositories page.
...
- shows the proposed Whitesmith label set beside each current value;
