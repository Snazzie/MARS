...
- Preserve existing cleanup behavior unless `WHITESMITH_DEBUG_PRESERVE_LEASES=1` is set.
...
**Interfaces:**
- Environment variable: `WHITESMITH_DEBUG_PRESERVE_LEASES=1`.
...
- [ ] **Step 1: Add a failing preservation test**
  Set `Bun.env.WHITESMITH_DEBUG_PRESERVE_LEASES='1'`, run the lifecycle with spies, and assert neither cleanup method is called; restore the environment in `finally`.
...
- [ ] **Step 1: Enable preservation only in the current local process environment**
  Launch the orchestrator/control-plane with `WHITESMITH_DEBUG_PRESERVE_LEASES=1`; do not commit this setting as a default.
...
- [ ] **Step 5: Disable preservation after investigation**
  Remove/unset `WHITESMITH_DEBUG_PRESERVE_LEASES` before returning normal cleanup to production behavior.
