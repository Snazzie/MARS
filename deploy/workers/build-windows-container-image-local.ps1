...
try {
  ...
  $manifest = [ordered]@{ schemaVersion = 1; baseImage = $BaseImage; runnerSha256 = $RunnerSha256.ToLowerInvariant(); gitSha256 = $GitSha256.ToLowerInvariant(); vcRuntimeSha256 = $VcRuntimeSha256.ToLowerInvariant(); builderSha256 = (Get-FileHash -LiteralPath $PSCommandPath -Algorithm SHA256).Hash.ToLowerInvariant(); verifierSha256 = (Get-FileHash -LiteralPath $VerifierPath -Algorithm SHA256).Hash.ToLowerInvariant(); containerfileSha256 = (Get-FileHash -LiteralPath $ContainerfilePath -Algorithm SHA256).Hash.ToLowerInvariant(); entrypointSha256 = (Get-FileHash -LiteralPath $EntrypointPath -Algorithm SHA256).Hash.ToLowerInvariant(); jobAgentSha256 = (Get-FileHash -LiteralPath $JobAgent -Algorithm SHA256).Hash.ToLowerInvariant(); image = $Image; imageId = $imageId; runtimeProbe = $runtimeProbe; builtAt = (Get-Date).ToUniversalTime().ToString('o') }
