...
$name = "mars-linux-template-$([guid]::NewGuid().ToString('N'))"; $temp = Join-Path ([IO.Path]::GetTempPath()) $name; $working = Join-Path $temp 'template.vhdx'; New-Item -ItemType Directory -Force -Path $temp | Out-Null; Copy-Item $SourceVhdx $working
try {
  ...
  Run 'scp.exe' @('-i',$SshKeyPath,'-o','StrictHostKeyChecking=no',$JobAgentPath,"${GuestUser}@${GuestAddress}:/tmp/mars-job-agent")
  $remote = "sudo install -D -m 0755 /tmp/mars-job-agent /usr/local/bin/mars-job-agent; sudo install -d -m 0700 /var/lib/mars; sudo touch /var/lib/mars/guest-service.ready; sudo tee /etc/systemd/system/mars-guest.service >/dev/null <<'UNIT'`n[Unit]`nAfter=network-online.target`n[Service]`nType=simple`nExecStart=/usr/local/bin/mars-job-agent guest-service --platform linux-x64 --bootstrap-file /var/lib/mars/bootstrap.json`nRestart=on-failure`n[Install]`nWantedBy=multi-user.target`nUNIT`nsudo systemctl enable mars-guest.service; sudo rm -f /tmp/mars-job-agent /etc/ssh/ssh_host_*; sudo cloud-init clean --logs 2>/dev/null || true; sudo shutdown -h now"
