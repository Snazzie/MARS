...
test("verifies manifest and every hashed asset into normalized binding data", async () => {
  await expect(verifyPublishedWorkerRelease(manifestUrl, "0.2.0", { fetch: fetcher() })).resolves.toMatchObject({ workerTag: "worker-v0.1.1", workerVersion: "0.1.1", manifestUrl, contractVersion: "0.2.0", workerBuildId: "worker-build", brokerDigest: `sha256:${"a".repeat(64)}`, armBrokerDigest: `sha256:${"b".repeat(64)}`, armJobDigest: `sha256:${"c".repeat(64)}` });
});

test("rejects mutable, missing, inaccessible, mismatched, or incompatible releases", async () => {
  await expect(verifyPublishedWorkerRelease("https://github.com/Snazzie/MARS/releases/latest/download/worker-release-manifest.json", "0.2.0", { fetch: fetcher() })).rejects.toThrow("immutable");
  await expect(verifyPublishedWorkerRelease(manifestUrl, "0.2.0", { fetch: async () => new Response("missing", { status: 404 }) })).rejects.toThrow("HTTP 404");
  await expect(verifyPublishedWorkerRelease(manifestUrl, "0.2.0", { fetch: fetcher({ missing: "installer.sh" }) })).rejects.toThrow("HTTP 404");
  await expect(verifyPublishedWorkerRelease(manifestUrl, "0.2.0", { fetch: fetcher({ mismatch: "installer.sh" }) })).rejects.toThrow("SHA-256 mismatch");
  const withoutLinux = { ...manifest, platforms: { ...manifest.platforms, "linux-x64": null } };
  await expect(verifyPublishedWorkerRelease(manifestUrl, "0.2.0", { fetch: async () => new Response(JSON.stringify(withoutLinux)) })).rejects.toThrow("linux-x64");
  const withoutArm = { ...manifest, platforms: { ...manifest.platforms, "linux-arm64": null } };
  await expect(verifyPublishedWorkerRelease(manifestUrl, "0.2.0", { fetch: async () => new Response(JSON.stringify(withoutArm)) })).rejects.toThrow("linux-arm64");
  await expect(verifyPublishedWorkerRelease(manifestUrl, "0.3.0", { fetch: fetcher() })).rejects.toThrow("incompatible");
});
