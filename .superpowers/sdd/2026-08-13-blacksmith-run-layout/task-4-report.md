...
- Added run-scoped graphite/blue/pink tokens under `.runs-heading`, `.run-history`, and `.run-detail-grid`; token scope explicitly covers the history sibling composition without affecting other routes.
...
- Browser computed-style probe confirmed run tokens resolve on the history composition: `--run-graphite=#161819`, `--run-panel=#1c1e20`, `--run-blue=#4f83ff`, `--run-pink=#ff4f83`.
