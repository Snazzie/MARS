...
- Final verification: `bun test apps/control-plane/src/runs.test.ts apps/control-plane/src/github-jobs.test.ts` — 6 passed, 20 assertions; `bun run --filter @mars/control-plane typecheck` — passed.
- Follow-up review coverage: assertions now inspect stored run rows after queued, in-progress, completed, and stale replay updates; differing timestamps prove earliest start and first completion behavior for both run/job state.
- Final step timestamp coverage: out-of-order completed step updates assert persisted `started_at` keeps the earliest value and `completed_at` keeps the first terminal value.